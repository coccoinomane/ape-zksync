...
class ApeZksyncError(ApeException):
    """
    Raised in the ape-zksync plugin.
    """
...
class Zksync(Ethereum):
    ...
    def config(self) -> ZksyncConfig:  # type: ignore
        return cast(ZksyncConfig, self.config_manager.get_config("zksync"))
...
def _get_transaction_cls(transaction_type: TransactionType) -> Type[TransactionAPI]:
    ...
    if transaction_type not in transaction_types:
        raise ApeZksyncError(f"Transaction type '{transaction_type}' not supported.")
